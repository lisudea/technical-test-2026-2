...
<template>
  <nav v-if="totalPages > 1" aria-label="Equipment pagination">
    <ul class="pagination justify-content-center flex-wrap">
      ...
      <li class="page-item disabled" v-if="visiblePages[0] > 1"><span class="page-link">...</span></li>
      <li v-for="p in visiblePages" :key="p" class="page-item" :class="{ active: p === page }">
        <button class="page-link" @click="emit('update:page', p)">{{ p + 1 }}</button>
      </li>
      ...
      <li class="page-item" v-if="visiblePages[visiblePages.length - 1] < totalPages - 1">
        <button class="page-link" @click="emit('update:page', totalPages - 1)">{{ totalPages }}</button>
      </li>

...
public interface ReservaRepository extends JpaRepository<Reserva, Long> {
    ...
    // Este método lista las reservas de un equipo con un estado específico.
    Page<Reserva> findByEquipoIdAndEstado(Long equipoId, EstadoReserva estado, Pageable pageable);

    // Este método busca las reservas activas cuya fecha de devolución ya pasó, para marcarlas como finalizadas.
    // Se utiliza en la transición de estado FINALIZADA: una reserva activa pasa a finalizada cuando su fecha de
    // devolución es anterior a la fecha actual.
    List<Reserva> findByEstadoAndFechaDevolucionBefore(EstadoReserva estado, LocalDateTime fecha);

    // Este método consulta los equipos más solicitados para generar estadísticas y los resultados se ordenan de mayor 
    // a menor según la cantidad de reservas activas y finalizadas (las canceladas no se cuentan). El límite de resultados
    // se controla con el Pageable. Se guarda el id del equipo y la cantidad de reservas asociadas a ese equipo en un arreglo
    // de objetos.

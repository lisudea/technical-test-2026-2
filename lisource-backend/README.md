...
<p align="center">
  <img src="docs/assets/logo-lis.png" alt="Logo LIS" width="160">
</p>
...
[![Spring Boot 3.5.16](https://img.shields.io/badge/Spring_Boot-3.5.16-6DB33F?logo=springboot&logoColor=white)](pom.xml)
[![Spring Security](https://img.shields.io/badge/Spring_Security-implemented-6DB33F?logo=springsecurity&logoColor=white)](pom.xml)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Supabase-316192?logo=postgresql&logoColor=white)](src/main/resources/db)
...
| **Gestión de equipos** | Registrar, actualizar y visualizar equipos con ID único, nombre, serie/MAC, categoría y estado | [EquipmentController](src/main/java/co/edu/udea/lis/lisource/equipment/api/EquipmentController.java) | crear, editar, listar y consultar un equipo en Swagger/Postman |
...
| **Gestión de reservas** | Crear, cancelar y listar reservas asociadas al usuario y a uno o varios equipos, con inicio y fin | [ReservationService](src/main/java/co/edu/udea/lis/lisource/reservation/application/ReservationService.java) | crear una reserva, consultar `/me` y cancelarla |
...
|---|---|---|---|
| Top 5 | Calcula el ranking histórico de equipos más reservados | [StatisticsController](src/main/java/co/edu/udea/lis/lisource/statistics/api/StatisticsController.java) | consultar `GET /api/v1/statistics/top-equipment?limit=5` |
| Google SSO | Ingreso institucional con validación de `id_token` | [AuthService](src/main/java/co/edu/udea/lis/lisource/auth/application/AuthService.java) | login Google con correo `@udea.edu.co` |
| Dominio `@udea.edu.co` | Bloquea dominios fuera de la universidad | [EmailDomainPolicyTest](src/test/java/co/edu/udea/lis/lisource/auth/application/EmailDomainPolicyTest.java) | intentar con una cuenta no institucional |
| JWT | Access corto + refresh HttpOnly | [TokenCodecTest](src/test/java/co/edu/udea/lis/lisource/shared/security/TokenCodecTest.java) | login, copiar solo `accessToken` y autorizar Swagger |
...
|---|---|---|---|
| Sesiones | Lista, revoca y cierra sesiones activas | [ProfileController](src/main/java/co/edu/udea/lis/lisource/user/api/ProfileController.java), [AuthService](src/main/java/co/edu/udea/lis/lisource/auth/application/AuthService.java) | abrir `GET /api/v1/sessions` y revocar una sesión |
| Perfil | Consulta y actualización de nombre e idioma | [ProfileController](src/main/java/co/edu/udea/lis/lisource/user/api/ProfileController.java) | abrir `/api/v1/profile` y guardar cambios |
| Recuperación | Solicita y aplica reinicio de contraseña | [AuthService](src/main/java/co/edu/udea/lis/lisource/auth/application/AuthService.java) | usar `forgot-password` y `reset-password` |
| Múltiples roles | Permite seleccionar y cambiar rol activo | [AuthService](src/main/java/co/edu/udea/lis/lisource/auth/application/AuthService.java) | probar `select-role` y `switch-role` con usuario dual |
| Storage | Sube o elimina imágenes de equipos en Supabase Storage | [EquipmentImageStorage](src/main/java/co/edu/udea/lis/lisource/equipment/application/EquipmentImageStorage.java) | cargar una imagen desde el endpoint de equipo |
| Administración | Gestiona usuarios, roles, categorías, ubicaciones y configuración | [AdminUserService](src/main/java/co/edu/udea/lis/lisource/user/application/AdminUserService.java), [AdminCatalogService](src/main/java/co/edu/udea/lis/lisource/catalog/application/AdminCatalogService.java), [ConfigurationService](src/main/java/co/edu/udea/lis/lisource/configuration/application/ConfigurationService.java) | abrir endpoints `/api/v1/admin/*` en Swagger |
| Dashboard | Resume inventario y actividad operativa | [StatisticsController](src/main/java/co/edu/udea/lis/lisource/statistics/api/StatisticsController.java) | consultar `/api/v1/dashboard/summary` |
| Auditoría | Registra eventos con metadatos y trazabilidad | [AuditController](src/main/java/co/edu/udea/lis/lisource/audit/api/AuditController.java) | consultar `/api/v1/admin/audit` |
| Correlation ID | Propaga trazabilidad de una petición a otra | [CorrelationIdFilter](src/main/java/co/edu/udea/lis/lisource/shared/web/CorrelationIdFilter.java) | enviar `X-Correlation-ID` y revisarlo en respuesta/logs |
| Realtime | Publica eventos sobre cambios relevantes | [WebSocketConfig](src/main/java/co/edu/udea/lis/lisource/shared/config/WebSocketConfig.java) | conectar cliente STOMP y verificar publicación |
...
![Modelo relacional de LISource](docs/assets/database/modelo-relacional.png)
...
![Backend DevSecOps pipeline](docs/assets/evidence/backend/ci-cd/01-backend-devsecops-pipeline-success.png)

![Backend artifacts](docs/assets/evidence/backend/ci-cd/02-backend-artifacts.png)
...
- modelo relacional: [imagen base de datos](docs/assets/database/modelo-relacional.png)
- CI/CD backend: [pipeline](docs/assets/evidence/backend/ci-cd/01-backend-devsecops-pipeline-success.png) y [artefactos](docs/assets/evidence/backend/ci-cd/02-backend-artifacts.png)
- cloud compartido: [Terraform init/validate](docs/assets/evidence/shared/cloud/01-aws-terraform-init-validate.png) y [Terraform apply / OIDC](docs/assets/evidence/shared/cloud/02-aws-terraform-apply-oidc-roles.png)

...
`.github/workflows/frontend-ci.yml` usa Node 22 y lockfile. CodeQL requiere `security-events: write`; Trivy publica SARIF, informa HIGH/CRITICAL y bloquea CRITICAL corregible según configuración real. La imagen `lisource-frontend-image` se conserva un día como artefacto inter-job. No se inventan CVEs: revise el run concreto.
